...
class createpurchaseorder(models.TransientModel):
    ...
    new_order_line_ids = fields.One2many('getsale.orderdata', 'new_order_line_id', string="Sipariş Satırları")
    partner_id = fields.Many2one('res.partner', string='Tedarikçi', required=True)
    date_order = fields.Datetime(string='Siariş Tarihi', required=True, copy=False, default=fields.Datetime.now)
    ...
    def action_create_purchase_order(self):
        ...
        message = "Satınalma siparişi oluşturuldu "+'<a href="#" data-oe-id=' + str(
...
class Getsaleorderdata(models.TransientModel):
    ...
    product_qty = fields.Float(string='Adet', required=True)
    date_planned = fields.Datetime(string='Planlanan Tarih', default=datetime.today())
    product_uom = fields.Many2one('uom.uom', string='Ürün Birimi')
    order_id = fields.Many2one('sale.order', string='Siariş Referansı', ondelete='cascade', index=True)
    price_unit = fields.Float(string='Birim Fiyatı', digits='Product Price')
